...
export default function CriticalProjectsTimeline({ data }: CriticalProjectsTimelineProps) {
  const timelineData = useMemo(() => {
    ...
    const criticalProjects = data.filter(project => 
      project['DCA 2025'] === 'Low' || project['DCA 2025'] === 'Medium-Low'
    );

    // Process projects with detailed information for timeline
    const processedProjects = criticalProjects.map(project => {
        ...
        for (const format of dateFormats) {
          ...
          if (match) {
            if (format === dateFormats[0] || format === dateFormats[1]) {
              ...
              // YYYY-MM-DD
              endDate = new Date(match[1] + '-' + match[2] + '-' + match[3]);
              year = match[1];
            } else if (format === dateFormats[3]) {
              // Just year
              endDate = new Date(parseInt(match[1]), 11, 31);
              year = match[1];
            }
            break;
          }
      ...
      return {
        ...
        endDate,
        year,
        budget: project['Total budget (millions)'] || 0,
